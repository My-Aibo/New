...
export function formatNumber(
  ...
  if (value >= 1e12) {
    return `${type === "currency" ? "$" : ""}${(value / 1e12).toFixed(
      decimals
    )}T`;
  }
  if (value >= 1e9) {
    return `${type === "currency" ? "$" : ""}${(value / 1e9).toFixed(
      decimals
    )}B`;
  }
  if (value >= 1e6) {
    return `${type === "currency" ? "$" : ""}${(value / 1e6).toFixed(
      decimals
    )}M`;
  }
  if (value >= 1e3) {
    return `${type === "currency" ? "$" : ""}${(value / 1e3).toFixed(
      decimals
    )}K`;
  }
